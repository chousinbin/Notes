...
3. 当创建子类对象时, 不管使用子类的哪个构造器, 默认情况下总会去调用父类的无参构造器, 如果父类没有提供无参构造器, 则必须在子类的构造器中用super去指定父类中的构造器完成对父类的初始化, 否则编译报错.

4. 如果希望指定去调用父类的某个构造器, 则用super显示的调用一下

5. super必须放在构造器的第一行(为了先初始化父类), 只能在构造器中使用

6. super()和this() 都只能放在构造器的第一行, 因此这两个方法不能同时存在于一个构造器中
